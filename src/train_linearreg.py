...
if __name__ == "__main__":
    ...
    ap.add_argument("--out",  default="models/LinearRegression.joblib")
    args = ap.parse_args()
    ...
    # Target encoding of location (based on TRAIN only)
    ...
    # Explicit feature choice for LINEAR REGRESSION
    ...
    valid = valid.dropna(subset=FEATURES_LR + [TARGET])
    test  = test.dropna(subset=FEATURES_LR + [TARGET])

    Xtr, ytr = train[FEATURES_LR], train[TARGET]
    Xva, yva = valid[FEATURES_LR], valid[TARGET]
    Xte, yte = test[FEATURES_LR],  test[TARGET]
    ...
    joblib.dump(
        {"pipeline": pipe, "features": FEATURES_LR, "y": TARGET},
        args.out
    )
